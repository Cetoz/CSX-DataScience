...
# Products ----------------------------------------------------------------
#prod_first_orders 屬於某人的第一筆訂單
#prod_second_orders 屬於某人的第二筆訂單
prd <- orders_products %>%
...
# Users -------------------------------------------------------------------
#user_orders 該用戶總訂單
#user_period 該用戶從第一筆訂單到最後一筆訂單總時長
#user_mean_days_prior_order 兩筆訂單平均間隔
users <- orders %>%
...
us <- orders %>%
  filter(eval_set != "prior") %>%#挑出非prior資料
